...
export interface FileSystemFixtures {
  createdFiles: string[];
  createdDirs: string[];
  createTestFile?: (_filePath: string, _content?: string) => void;
  createTestDirectory?: (_dirPath: string) => void;
}
...
export const setupFileSystemFixture = (): FileSystemFixtures & { cleanup: () => void } => {
  ...
  const createTestFile = (filePath: string, content = ''): void => {
...
export const setupVSCodeFixture = (): {
  createSettings: (_settings: Record<string, unknown>) => void;
  createExtensions: (_extensions: string[]) => void;
  cleanup: () => void;
} => {
...
export const setupPerformanceTimer = (): {
  ...
  measure: (_fn: () => Promise<unknown> | unknown) => Promise<number>;
} => {
