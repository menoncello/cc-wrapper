...
} from '../test-utils/fixtures/setup-fixtures';
...
describe('Setup Performance - P0 Critical SLA Validation', () => {
  ...
  describe('Health Check Performance', () => {
    ...
    test('should measure individual service check performance', async () => {
      ...
      for (const checkMethod of checks) {
        const duration = await timer.measure(async () => {
          try {
            const checkerWithMethods = checker as unknown as Record<string, () => Promise<unknown>>;
            await checkerWithMethods[checkMethod]();
          } catch {
  ...
  describe('Performance Regression Detection', () => {
    test('should establish performance baseline', async () => {
      ...
      const avgDuration =
        baseline.environmentCheck.reduce((sum, time) => sum + time, 0) /
        baseline.environmentCheck.length;
