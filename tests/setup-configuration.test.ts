...
import { setupMockConsole, setupFileSystemFixture, setupVSCodeFixture } from '../test-utils/fixtures/setup-fixtures'
import { createMockEnvironmentConfig } from '../test-utils/factories/setup-factory'
...
describe('Environment Configuration - P0 Critical Setup Validation', () => {
  ...
  describe('Configuration Integration', () => {
    test('should validate complete environment setup', async () => {
      ...
      // Cleanup
      ['.env.local', 'docker-compose.dev.yml', '.vscode', 'apps', 'packages', 'services'].forEach(path => {
        if (fs.existsSync(path)) {
          if (fs.statSync(path).isDirectory()) {
            fs.rmSync(path, { recursive: true, force: true })
          } else {
